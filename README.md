...
└── minecraft/
    ...
    └── data/                   # Runtime data (gitignored, persistent)
        ...
        ├── mods/               # All mod jars (pack + MODRINTH_PROJECTS)
...
8. Sets ownership to `1000:1000` (the itzg image's default user).

> Steps 1-7 only run on first boot (when `/data/run.sh` doesn't exist). Step 8 runs on **every** boot. Extra mods are downloaded by the itzg image via `MODRINTH_PROJECTS` — see [Boot Sequence](#boot-sequence).
...
2. **Config copy** (always runs) — copies tracked `server.properties` and `server-icon.png` into `/data/`.

Extra mods are no longer handled by setup.sh — the itzg image auto-downloads them via `MODRINTH_PROJECTS` at startup. Setup finishes in seconds, then the server starts.
...
- **`docker compose down && up`** — Phase 1 skipped (data persists). Extra mods are re-synced by the itzg image via `MODRINTH_PROJECTS`. Updated mod lists in `docker-compose.yml` take effect.
...
Extra server-side mods are managed by the itzg image's `MODRINTH_PROJECTS` environment variable in `docker-compose.yml`. The image auto-downloads the latest compatible versions at boot and cleans up stale jars — no binaries committed to git.

### Configuration

In `docker-compose.yml`, the `minecraft` service environment:
...
`VERSION` and `MODRINTH_LOADER` are needed because `TYPE=CUSTOM` means the image can't infer the modloader from `TYPE`. Each entry is a Modrinth project slug; append `:beta` for beta-only mods. The itzg image handles version resolution, download, and stale-jar cleanup automatically.

### Currently included mods

| Mod | Slug | Purpose |
|---|---|---|
| **Chunky** | `chunky` | World pre-generation — eliminates chunk-gen lag |
...
| Mod | Slug | Reason |
|---|---|---|
| Lithium | `lithium` | Commented out — not needed with current modpack |
| FerriteCore | `ferritecore` | Commented out — not needed with current modpack |
| AI Improvements | `ai-improvements` | Listed range is 1.21–1.21.4, not 1.21.1-exact |
| Alternate Current | `alternate-current` | Listed range is 1.21–1.21.1 |
| Clumps | `clumps` | Commented out — not needed with current modpack |

### Adding a new mod

1. Find the mod on [Modrinth](https://modrinth.com).
2. Make sure it's the **NeoForge** version for **1.21.1**.
3. Copy the project slug from the URL (e.g. `modrinth.com/mod/chunky` → `chunky`).
4. Add the slug to `MODRINTH_PROJECTS` in `docker-compose.yml`. Append `:beta` if the mod only has beta releases.
5. Commit and push.
6. Apply: `docker compose down && docker compose up -d` (no wipe needed).

To pin a specific version, use `slug:versionID` (e.g. `chunky:LuFhm4eU`). The version ID is on the Modrinth version page.

### Removing a mod

1. Delete the slug from `MODRINTH_PROJECTS` in `docker-compose.yml`.
2. Commit and push.
3. `docker compose down && docker compose up -d` — the itzg image removes the stale jar automatically.
