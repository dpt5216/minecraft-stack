# Minecraft Server Stack — "DeanPAC"

A fully reproducible, Docker-based Minecraft server running the **DeanPAC** custom modpack (v1.7) on **NeoForge 21.1.228** for **Minecraft 1.21.1**. Zero-friction redeploys: wipe the data directory, run `docker compose up`, and the server rebuilds itself — no manual jar downloads, no API keys.
...
| **minecraft-setup** | `eclipse-temurin:21` | One-shot init: downloads NeoForge installer, downloads 283 mod JARs from CurseForge CDN, copies tracked configs, pulls extra mods + datapacks | Runs once on fresh install; Phases 2-4 re-run on every `compose down → up` |
...
1. Downloads the **NeoForge installer** directly from the NeoForge Maven repository (no API key needed).
2. Runs `--installServer` to generate the actual server launcher (`run.sh`).
3. Downloads **283 mod JARs** from CurseForge's CDN (edge.forgecdn.net) using URLs extracted from the DeanPAC `minecraftinstance.json` manifest — no API key, no zip extraction.
4. Copies the **tracked config tree** (497 files) from the repo into `/data/config/`.
5. Downloads **extra server-side mods** (Chunky, Spark) and **6 server-side datapacks**.
6. Applies our tracked `server.properties` and `server-icon.png`.
...
└── minecraft/
    ├── setup.sh                # Init container script (four-phase)
    ├── server.properties       # Tracked server config
    ├── server-icon.png         # Tracked server list icon (64x64 PNG)
    ├── deanpac-mods.txt        # Download URLs for DeanPAC mods (283 jars)
    ├── deanpac-datapacks.txt   # Download URLs for server-side datapacks (6 zips)
    ├── extra-mods.txt          # Download URLs for extra server-side mods (Chunky, Spark)
    ├── configs/                # Tracked DeanPAC config tree (497 files)
    └── data/                   # Runtime data (gitignored, persistent)
        ├── run.sh              # Generated by NeoForge installer
        ├── libraries/          # NeoForge runtime libraries
        ├── mods/               # All mod jars (DeanPAC + extra-mods.txt)
...
Everything in `minecraft/` **except** `data/` is git-tracked — including the 497-file `configs/` tree (5.8 MB) and the 283-URL `deanpac-mods.txt`. The `data/` directory is the persistent runtime state — it survives container restarts but is wiped during a full rebuild.
...
2. **DeanPAC mods** (always runs) — reads `deanpac-mods.txt`, removes stale jars via manifest, re-downloads all 283 mods. URL changes take effect with `compose down -> git pull -> up` — no wipe needed.
3. **Extra mods** (always runs) — reads `extra-mods.txt`, removes stale jars via manifest, re-downloads Chunky + Spark.
4. **Config sync** (always runs) — copies the tracked config tree from `minecraft/configs/` into `/data/config/`, replacing the previous copy.
5. **Datapack sync** (always runs) — re-downloads the 6 datapacks into `/data/world/datapacks/`.

Setup finishes in ~5 minutes (the mod downloads), then the server starts.
...
docker compose up -d
        ...
        │         │     │     ├── YES → skip to Phase 2
        │         │     │     └── NO  → continue ↓
        │         │     ├── wget NeoForge installer from maven.neoforged.net
        │         │     ├── rm -rf /data/*          (clean slate)
        │         │     ├── java -jar neoforge-installer.jar --installServer
        │         │     └── cp server.properties + server-icon.png → /data/
        │         │
        │         ├── Phase 2: DeanPAC mods (always runs)
        │         │     ├── remove jars from previous run (via manifest)
        │         │     ├── wget each URL from deanpac-mods.txt → /data/mods/
        │         │     └── write manifest for next boot
        │         │
        │         ├── Phase 2b: Extra mods (always runs)
        │         │     ├── remove jars from previous run (via manifest)
        │         │     ├── wget each URL from extra-mods.txt → /data/mods/
        │         │     └── write manifest for next boot
        │         │
        │         ├── Phase 3: Config sync (always runs)
        │         │     ├── rm -rf /data/config
        │         │     └── cp -r /deanpac-config → /data/config
        │         │
        │         ├── Phase 4: Datapack sync (always runs)
        │         │     ├── wget each URL from deanpac-datapacks.txt
...
- **`docker compose down && up`** — Phase 1 skipped (data persists), Phases 2-4 re-run. Updated URLs in `deanpac-mods.txt`, `extra-mods.txt`, or `deanpac-datapacks.txt`, and config changes in `minecraft/configs/`, all take effect.
- **`rm -rf minecraft/data/* && docker compose up`** — full rebuild from scratch (all phases).
...
|---|---|---|
| `motd` | `§5§lDean's Modded Minecraft §r§d— DeanPAC v1.7` | Uses Minecraft color codes |
...
The `minecraft/extra-mods.txt` file contains a list of direct-download URLs for server-side mods **not already included in the DeanPAC mod list**. These are downloaded into `/data/mods/` during setup and **re-synced on every boot** — no binaries committed to git. Adding, updating, or removing a mod only requires editing the file and running `docker compose down && up`.
...
> **Note:** Lithium, FerriteCore, ModernFix (5.27.8), and Distant Horizons are all included in the DeanPAC mod list (`deanpac-mods.txt`) and do not need to be listed here. Keeping them out of `extra-mods.txt` avoids duplicate jar downloads.
...
> ⚠️ The DH LOD database lives in `minecraft/data/world/data/DistantHorizons.sqlite` and survives restarts but **not** a wipe. Back it up alongside the world if you want to avoid re-running pregen after a rebuild. Distant Horizons is now included in the DeanPAC mod list.
...
These mods are included in the DeanPAC mod list (`deanpac-mods.txt`) and downloaded automatically during setup:
...
| **Distant Horizons** | Server-side LOD generation + streaming to DH clients. | `/dh pregen` |

**Chunky** (world pre-generation) is not in DeanPAC — it's in `extra-mods.txt`.
...
Stops the server, tars the entire `minecraft/data/` directory (world, mods, config, DH LODs, ops.json, whitelist), restarts. Use this for a complete snapshot before a DeanPAC update or major change.
...
> 💡 **Full restore and setup.sh:** When restoring a full backup, `setup.sh` sees `/data/run.sh` already exists and skips the NeoForge install (Phase 1). It still re-syncs DeanPAC mods, extra mods, configs, and datapacks (Phases 2-4). Your world, mods, and config are preserved from the backup.
...
When a new version of DeanPAC is released:

1. **Regenerate the mod URL list.**
   - Export the new DeanPAC instance from CurseForge (or obtain the updated `minecraftinstance.json`)
   - Run the generation script against the new JSON to produce an updated `deanpac-mods.txt`
   - The script filters out client-only mods and duplicates automatically

2. **Update the config tree.**
   - Copy the new instance's `config/` folder into `minecraft/configs/` (excluding `.bak` files)
   - Commit the changes

3. **Check for NeoForge version changes.**
   - If the new DeanPAC version uses a different NeoForge, update `NEOFORGE_VERSION` in `minecraft/setup.sh`
   - The installer URL is constructed from this variable

4. **Update the MOTD** in `minecraft/server.properties` if the version changed.

5. **Back up the world** (see [Backups & Restore](#backups--restore)).
...
7. **Restore the world** if you backed it up:
...
8. **Update the landing page** in `site/index.html` with the new version number.
...
2. Installing the DeanPAC modpack
